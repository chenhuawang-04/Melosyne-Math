...
constexpr bool kHasGlm = FM_HAVE_GLM;
constexpr bool kHasEigen = FM_HAVE_EIGEN;
constexpr bool kHasDirectXMath = FM_HAVE_DIRECTXMATH;

#if FM_HAVE_GLM
inline glm::vec3 to_glm(const MMath::Vec3& v) { return glm::vec3(v.x, v.y, v.z); }
inline glm::vec4 to_glm(const MMath::Vec4& v) { return glm::vec4(v.x, v.y, v.z, v.w); }
...
inline Eigen::Vector4f to_eigen(const MMath::Vec4& v) { return Eigen::Vector4f(v.x, v.y, v.z, v.w); }
...
#if FM_HAVE_DIRECTXMATH
inline DirectX::XMVECTOR to_dxm(const MMath::Vec3& v) {
    return DirectX::XMVectorSet(v.x, v.y, v.z, 0.0f);
}
inline DirectX::XMVECTOR to_dxm(const MMath::Vec4& v) {
