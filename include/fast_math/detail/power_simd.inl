...
inline void powSimd(
    ...
    // Scalar cleanup for special-domain inputs that the SIMD log path does not
    // model faithfully (negative base, zero base, infinities, NaNs).
    for (i = 0; i < count_; ++i) {
        if (!(base_[i] > 0.0f) || !std::isfinite(base_[i]) || !std::isfinite(exponent_[i])) {
            result_[i] = MMath::pow(base_[i], exponent_[i]);
        }
    }
}
