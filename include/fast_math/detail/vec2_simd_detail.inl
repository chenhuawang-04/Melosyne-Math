// Shared Vec2 SIMD batch/detail implementation for header/module parity.

namespace detail {

MMATH_FORCE_INLINE Vec2 vec2AddScalar(Vec2 a_, Vec2 b_) noexcept {
    return Vec2{ a_.x + b_.x, a_.y + b_.y };
}

MMATH_FORCE_INLINE float vec2DotScalar(Vec2 a_, Vec2 b_) noexcept {
    return a_.x * b_.x + a_.y * b_.y;
}

MMATH_FORCE_INLINE Vec2 vec2NormalizeScalar(Vec2 v_) noexcept {
    float len_sq = v_.x * v_.x + v_.y * v_.y;
    if (len_sq > 1e-8f) {
        float inv_len = 1.0f / std::sqrt(len_sq);
        return Vec2{ v_.x * inv_len, v_.y * inv_len };
    }
    return Vec2{ 0.0f, 0.0f };
}

MMATH_FORCE_INLINE Vec2 vec2NormalizeFastScalar(Vec2 v_) noexcept {
    float len_sq = v_.x * v_.x + v_.y * v_.y;
    if (len_sq > 1e-8f) {
        float inv_len = 1.0f / std::sqrt(len_sq);
        return Vec2{ v_.x * inv_len, v_.y * inv_len };
...
inline void vec2AddSimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        result_[i] = vec2AddScalar(a_[i], b_[i]);
    }
}
...
inline void vec2DotSimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        result_[i] = vec2DotScalar(a_[i], b_[i]);
    }
}
...
inline void vec2NormalizeSimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        result_[i] = vec2NormalizeScalar(input_[i]);
    }
}
...
inline void vec2NormalizeFastSimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        result_[i] = vec2NormalizeFastScalar(input_[i]);
    }
}
