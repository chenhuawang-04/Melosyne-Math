// Shared lerp SIMD batch/detail implementation for header/module parity.

namespace detail {

MMATH_FORCE_INLINE float clampUnitScalarFallback(float x_) noexcept {
    float temp = (x_ < 1.0f) ? x_ : 1.0f;
    return (temp > 0.0f) ? temp : 0.0f;
}

MMATH_FORCE_INLINE float lerpScalarFallback(float a_, float b_, float t_) noexcept {
    return a_ + t_ * (b_ - a_);
}

MMATH_FORCE_INLINE float inverseLerpScalarFallback(float a_, float b_, float value_) noexcept {
    float range = b_ - a_;
    if ((range < 0.0f ? -range : range) < 1e-8f) {
        return 0.0f;
    }
    return (value_ - a_) / range;
}

MMATH_FORCE_INLINE float smoothstepScalarFallback(float edge0_, float edge1_, float t_) noexcept {
    float x = clampUnitScalarFallback((t_ - edge0_) / (edge1_ - edge0_));
    return x * x * (3.0f - 2.0f * x);
}

MMATH_FORCE_INLINE float smootherstepScalarFallback(float edge0_, float edge1_, float t_) noexcept {
    float x = clampUnitScalarFallback((t_ - edge0_) / (edge1_ - edge0_));
    return x * x * x * (x * (x * 6.0f - 15.0f) + 10.0f);
}

MMATH_FORCE_INLINE float remapScalarFallback(float value_, float in_min_, float in_max_, float out_min_, float out_max_) noexcept {
    float t = inverseLerpScalarFallback(in_min_, in_max_, value_);
    return lerpScalarFallback(out_min_, out_max_, t);
}
...
inline void lerpSimd(
    ...
    int32_t i = 0;
    [[maybe_unused]] const bool use_streaming = count_ > 16384;  // Use streaming stores for >64KB
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        result_[i] = lerpScalarFallback(a_[i], b_[i], t_);
    }
}
...
inline void smoothstepSimd(
    ...
    [[maybe_unused]] const float inv_range = 1.0f / (edge1_ - edge0_);
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        values_[i] = smoothstepScalarFallback(edge0_, edge1_, values_[i]);
    }
}
...
inline void smootherstepSimd(
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        values_[i] = smootherstepScalarFallback(edge0_, edge1_, values_[i]);
    }
}
...
inline void remapSimd(
    ...
    if ((in_range < 0.0f ? -in_range : in_range) < 1e-8f) {
    ...
    [[maybe_unused]] const float scale = out_range / in_range;
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        result_[i] = remapScalarFallback(values_[i], in_min_, in_max_, out_min_, out_max_);
    }
}
