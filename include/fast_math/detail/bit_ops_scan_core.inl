// Shared scalar bit scanning/select/rank implementation for header/module parity.

namespace detail {

[[nodiscard]] inline constexpr uint64_t scanLowBitsMask(std::size_t bit_count) noexcept {
    if (bit_count == 0) return 0;
    if (bit_count >= 64) return ~uint64_t{0};
    return (uint64_t{1} << bit_count) - 1;
}

[[nodiscard]] inline constexpr uint64_t scanLastWordMask(std::size_t bit_count) noexcept {
    const std::size_t remainder = bit_count % 64;
    return remainder == 0 ? ~uint64_t{0} : scanLowBitsMask(remainder);
}

[[nodiscard]] inline uint64_t scanMaskedWord(
    ConstBitSetView view, std::size_t word_idx) noexcept {

    if (word_idx >= view.word_count) return 0;

    uint64_t word = view.data[word_idx];
    if (word_idx + 1 == view.word_count) {
        word &= scanLastWordMask(view.bit_count);
    }
    return word;
}

[[nodiscard]] inline uint64_t scanMaskedWordForZero(
    ConstBitSetView view, std::size_t word_idx) noexcept {

    uint64_t word = scanMaskedWord(view, word_idx);
    if (word_idx < view.word_count && word_idx + 1 == view.word_count) {
        word |= ~scanLastWordMask(view.bit_count);
    }
    return word;
}

[[nodiscard]] inline constexpr uint64_t scanMaskInclusive(std::size_t bit_idx) noexcept {
    return bit_idx >= 63 ? ~uint64_t{0} : ((uint64_t{1} << (bit_idx + 1)) - 1);
}

[[nodiscard]] inline constexpr uint64_t scanMaskExclusive(std::size_t bit_idx) noexcept {
    return bit_idx == 0 ? 0 : ((uint64_t{1} << bit_idx) - 1);
}
...
[[nodiscard]] inline std::size_t findFirst(ConstBitSetView view) noexcept {
    for (std::size_t i = 0; i < view.word_count; ++i) {
        const uint64_t word = detail::scanMaskedWord(view, i);
        if (word != 0) {
            const std::size_t result = i * 64 + std::countr_zero(word);
...
[[nodiscard]] inline std::size_t findLast(ConstBitSetView view) noexcept {
    for (std::size_t i = view.word_count; i > 0; --i) {
        ...
        const uint64_t word = detail::scanMaskedWord(view, idx);
        if (word != 0) {
            const std::size_t leading_zeros = std::countl_zero(word);
...
[[nodiscard]] inline std::size_t findNext(
    ...
    uint64_t word = detail::scanMaskedWord(view, word_idx) & (~uint64_t{0} << bit_idx);
    if (word != 0) {
    ...
    for (++word_idx; word_idx < view.word_count; ++word_idx) {
        word = detail::scanMaskedWord(view, word_idx);
        if (word != 0) {
            std::size_t result = word_idx * 64 + std::countr_zero(word);
            return result < view.bit_count ? result : view.bit_count;
        }
...
[[nodiscard]] inline std::size_t findPrev(
    ...
    uint64_t word =
        detail::scanMaskedWord(view, word_idx) & detail::scanMaskInclusive(bit_idx);
    if (word != 0) {
    ...
    for (std::size_t i = word_idx; i > 0; --i) {
        const std::size_t idx = i - 1;
        word = detail::scanMaskedWord(view, idx);
        if (word != 0) {
            return idx * 64 + (63 - std::countl_zero(word));
        }
    }
...
[[nodiscard]] inline std::size_t findFirstZero(ConstBitSetView view) noexcept {
    for (std::size_t i = 0; i < view.word_count; ++i) {
        const uint64_t word = detail::scanMaskedWordForZero(view, i);
        if (word != ~uint64_t{0}) {
            const std::size_t pos = i * 64 + std::countr_zero(~word);
...
[[nodiscard]] inline std::size_t findLastZero(ConstBitSetView view) noexcept {
    for (std::size_t i = view.word_count; i > 0; --i) {
        const std::size_t idx = i - 1;
        const uint64_t word = detail::scanMaskedWordForZero(view, idx);
        if (word != ~uint64_t{0}) {
            const std::size_t leading_zeros = std::countl_zero(~word);
            const std::size_t pos = idx * 64 + (63 - leading_zeros);
            return pos < view.bit_count ? pos : view.bit_count;
        }
    }
    return view.bit_count;
...
// ============================================================================
// Rank operation: Count set bits up to position (exclusive)
// ============================================================================
...
[[nodiscard]] inline std::size_t rank(
    ...
    if (bit_idx > 0) {
        const uint64_t mask = detail::scanMaskExclusive(bit_idx);
        count += std::popcount(detail::scanMaskedWord(view, word_idx) & mask);
    }
