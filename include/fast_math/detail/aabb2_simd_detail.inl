// Shared Aabb2 SIMD batch/detail implementation for header/module parity.

namespace detail {

MMATH_FORCE_INLINE bool aabb2IntersectsScalar(Aabb2 a_, Aabb2 b_) noexcept {
    return !(a_.minx > -b_.neg_maxx || -a_.neg_maxx < b_.minx ||
             a_.miny > -b_.neg_maxy || -a_.neg_maxy < b_.miny);
}

MMATH_FORCE_INLINE Aabb2 aabb2MergeScalar(Aabb2 a_, Aabb2 b_) noexcept {
    return Aabb2{
        (a_.minx < b_.minx) ? a_.minx : b_.minx,
        (a_.miny < b_.miny) ? a_.miny : b_.miny,
        (a_.neg_maxx < b_.neg_maxx) ? a_.neg_maxx : b_.neg_maxx,
        (a_.neg_maxy < b_.neg_maxy) ? a_.neg_maxy : b_.neg_maxy
    };
}

MMATH_FORCE_INLINE bool aabb2ContainsPointScalar(Aabb2 aabb_, Vec2 p_) noexcept {
    float maxx = -aabb_.neg_maxx;
    float maxy = -aabb_.neg_maxy;
    return (p_.x >= aabb_.minx && p_.x <= maxx &&
            p_.y >= aabb_.miny && p_.y <= maxy);
}
...
inline void aabb2IntersectsArraySimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        results_[i] = aabb2IntersectsScalar(reference_, candidates_[i]);
    }
}
...
inline void aabb2MergeArraySimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        result_[i] = aabb2MergeScalar(a_[i], b_[i]);
    }
}
...
inline void aabb2ContainsPointsArraySimd(
    ...
    // Handle remainder
    for (; i < count_; ++i) {
        results_[i] = aabb2ContainsPointScalar(aabb_, points_[i]);
    }
}
