// Shared sqrt SIMD batch/detail implementation for header/module parity.

namespace detail {

MMATH_FORCE_INLINE float sqrtScalarFallback(float x_) noexcept {
    return std::sqrt(x_);
}

MMATH_FORCE_INLINE float inverseSqrtScalarFallback(float x_) noexcept {
    return 1.0f / std::sqrt(x_);
}

MMATH_FORCE_INLINE float reciprocalScalarFallback(float x_) noexcept {
    return 1.0f / x_;
}
...
inline void sqrtSimd(
    ...
    for (; i < count_; ++i) {
        values_[i] = sqrtScalarFallback(values_[i]);
    }
}
...
inline void inverseSqrtSimd(
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        values_[i] = inverseSqrtScalarFallback(values_[i]);
    }
}
...
inline void reciprocalSimd(
    ...
    // Scalar fallback
    for (; i < count_; ++i) {
        values_[i] = reciprocalScalarFallback(values_[i]);
    }
}
 ...
 * Accuracy: current tests validate a 2e-3f absolute-error budget on
 * representative positive finite inputs
 */
 ...
 * Accuracy: current tests validate a 2e-3f absolute-error budget on
 * representative non-zero finite inputs
 */
