// Shared sqrt scalar/core implementation for header/module parity.
 ...
 *
 * Implementation:
 * - SSE: rsqrtss (11-12 bit seed) + 1 Newton-Raphson iteration
 ...
 * - Current test budget: within 2e-3f absolute error on representative
 *   positive finite inputs exercised by the test suite
 *
 * @param x_ Input value (must be > 0)
 * @return 1/sqrt(x_)
 *
...
#if defined(__SSE4_1__)
    // Step 1: Get initial approximation (11-12 bit seed accuracy)
    __m128 v = _mm_set_ss(x_);
    __m128 y0 = _mm_rsqrt_ss(v);  // approximate 1/sqrt(x)

    // Step 2: Newton-Raphson iteration (refines to near single-precision accuracy)
 ...
 *
 * Implementation:
 * - SSE: rcpss (11-12 bit seed) + 1 Newton-Raphson iteration
 ...
 * - Current test budget: within 2e-3f absolute error on representative
 *   non-zero finite inputs exercised by the test suite
 *
...
MMATH_FORCE_INLINE float reciprocal(float x_) noexcept {
    ...
    // Step 1: Get initial approximation (11-12 bit seed accuracy)
    __m128 v = _mm_set_ss(x_);
    __m128 y0 = _mm_rcp_ss(v);  // approximate 1/x
